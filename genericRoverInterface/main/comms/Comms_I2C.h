...
extern "C" {
    ...
    private:
        ...
        // I2C Device
        std::map<uint16_t, i2c_master_dev_handle_t> device_list;

    public:
        I2CComms(std::shared_ptr<Logger> logger, i2c_port_num_t port = I2C_NUM_0, gpio_num_t data_pin = GPIO_NUM_21, gpio_num_t clk_pin = GPIO_NUM_22, i2c_clock_source_t clk_source = I2C_CLK_SRC_DEFAULT, uint8_t glitch_count = 7, uint32_t internal_pullup = true) :
        CommsBase(logger),
        i2c_port(port),
        sda_pin(data_pin),
        scl_pin(clk_pin),
        ...
        bool open(uint32_t dev_addr) override {
            ...
            if (it == device_list.end()){
                ...
                comms_logger->log_info("I2C", "Added new device. \n");
            }
